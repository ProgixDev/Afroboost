...
import { Text, Sheet, Button, EmptyState, BlurHeader, QueryBoundary } from '@/components/ui';
import { CallListItem } from '@/components/domain/CallListItem';
import { useCalls } from '@/lib/api';
...
export default function Calls() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: c.background }}>
      ...
      <QueryBoundary query={query} empty={{ icon: Phone, title: t('inbox.empty') }}>
        {(calls: Call[]) => (
          <FlatList
            data={calls}
            keyExtractor={(c) => c.id}
